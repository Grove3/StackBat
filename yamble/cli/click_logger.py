...
class ClickLogger:
    ...
    def __init__(self, verbose: bool = False, quiet: bool = False):
        ...
        if verbose and quiet:
            click.echo(
                click.style("Cannot use both verbose and quiet options", fg="red")
            )
            sys.exit(1)
    ...
    def log_generation_start(
        ...
        if not self.quiet:
            ...
            if self.verbose:
                ...
                if variables:
                    self._log_variables(variables, 4)
                if profiles:
                    self._log_profiles(profiles, 4)
    ...
    def log_templates(self, all_templates: Dict[str, Dict[str, Any]]):
        ...
        if self.verbose:
            total_templates = sum(
                len(templates) for templates in all_templates.values()
            )
            total_categories = len(
                [cat for cat, templates in all_templates.items() if templates]
            )
    ...
    def log_validation_errors(
        ...
        if validation_result.variable_errors:
            self.error(
                "The following variables input have errors, please check and try again:"
            )
    ...
    def _log_variables(self, variables: Dict[str, Dict[str, Any]], indent: int = 2):
        ...
        if self.verbose and variables:
            ...
            for template, var in variables.items():
                click.echo(f"{pad}{template}:")
                for key, value in var.items():
                    if isinstance(value, list):
                        click.echo(f"  {pad}{key}: [{', '.join(value)}]")
                    else:
                        click.echo(f"  {pad}{key}: {value}")

    def _log_profiles(self, profiles, indent: int = 2):
        """Log profiles in verbose mode"""
        pad = " " * indent
        if self.verbose and profiles:
            click.echo(click.style("  Profiles:", fg="yellow"))
            click.echo(f"{pad}Number: {profiles.get('number', 0)}")
            click.echo(f"{pad}Variables:")
    ...
    def _log_success(self, result: GenerationResult):
        ...
        if not self.quiet:
            ...
            click.echo(
                click.style(
                    "✅ Compose file generated successfully!", fg="green", bold=True
                )
            )
            ...
            if result.services_count:
                stats_parts.append(
                    f"{result.services_count} service{'s' if result.services_count != 1 else ''}"
                )
            if result.networks_count:
                stats_parts.append(
                    f"{result.networks_count} network{'s' if result.networks_count != 1 else ''}"
                )
            if result.volumes_count:
                stats_parts.append(
                    f"{result.volumes_count} volume{'s' if result.volumes_count != 1 else ''}"
                )
            ...
            if result.output_file:
                click.echo(
                    f"   File: {click.style(str(result.output_file), fg='cyan')}"
                )

    def _log_failure(self, result: GenerationResult):
        """Log failed generation"""
        click.echo(click.style("❌ Compose generation failed!", fg="red", bold=True))
        ...
        if result.processed_templates or result.skipped_templates:
            ...
            if result.skipped_templates:
                click.echo(
                    click.style(
                        f"   Skipped: {len(result.skipped_templates)} template{'s' if len(result.skipped_templates) != 1 else ''}",
                        fg="yellow",
                    )
                )
...
def quiet_option():
    ...
    return click.option(
        "--quiet", "-q", is_flag=True, help="Only show errors and critical information"
    )
